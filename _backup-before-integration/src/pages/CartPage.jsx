...
export default function CartPage() {
  const { cart, updateQuantity, setQuantity, removeItem, cartTotal, openCheckout } = useCart();
            ...
            {cartItems.map((item) => (
              <div key={item.id} className="flex flex-col md:grid md:grid-cols-12 gap-4 items-center bg-white p-4 border border-gray-100 rounded-lg shadow-sm">
                <div className="col-span-6 flex items-center gap-4">
                  <div className="w-20 h-20 bg-gray-50 rounded p-2 flex-shrink-0 border border-gray-200">
                    <img src="/MVP.png" alt={item.name} className="w-full h-full object-contain opacity-50" />
                  </div>
                  <div>
          ...
          <div className="lg:w-1/3">
            <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 sticky top-24">
              ...
              <div className="flex justify-between mb-6 text-gray-600">
                ...
                <span>Rs. {((cartTotal * 0.9) * 0.06).toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-2xl font-bold border-t pt-4 mb-6">
                <span>Total</span>
                <span>Rs. {((cartTotal * 0.9) * 1.06).toFixed(2)}</span>
              </div>
