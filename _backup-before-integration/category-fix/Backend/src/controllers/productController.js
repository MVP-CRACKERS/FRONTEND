...
const { getPriceListPdf, priceListFileName } = require('../services/priceListService');
...
/**
 * GET /api/products/price-list.pdf
 *
 * The downloadable price list: every active cracker with its image,
 * unit and current price. Public — it is the shop's catalogue.
 */
const downloadPriceList = asyncHandler(async (req, res) => {
  let result;
  try {
    result = await getPriceListPdf({ force: req.query.refresh === '1' && Boolean(req.admin) });
  } catch (err) {
    if (err.statusCode === 404) throw AppError.notFound(err.message);
    // eslint-disable-next-line no-console
    console.error('Price list generation failed:', err);
    throw new AppError(
      'We could not build the price list right now. Please try again in a moment, or message us on WhatsApp.',
      503,
  ...
  // The fingerprint changes whenever a price does, so browsers can cache
  // hard and still never serve a stale list.
  res.setHeader('ETag', `"${result.fingerprint}"`);
  res.setHeader('Cache-Control', 'public, max-age=300, must-revalidate');
  res.setHeader('X-Product-Count', String(result.productCount));
  res.setHeader('Access-Control-Expose-Headers', 'Content-Disposition, Content-Length, X-Product-Count');
  res.removeHeader('X-Frame-Options');
  res.setHeader('Content-Security-Policy', `frame-ancestors 'self' ${config.frontendUrls.join(' ')}`);

  if (req.headers['if-none-match'] === `"${result.fingerprint}"`) {
